...
router.post('/:id/step10', validateJWT, loadUser, async (req: Request, res: Response) => {
  try {
    ...
    // Drift-tolerant Step 9 done check — see services/stepGating.ts.
    if (!isStepDone(existingWorkflow, 9)) {
      return res.status(400).json({
        success: false,
...
router.post('/:id/step11', validateJWT, loadUser, async (req: Request, res: Response) => {
  try {
    ...
    // Drift-tolerant Step 10 done check — see services/stepGating.ts.
    if (!isStepDone(existingWorkflow, 10)) {
      return res.status(400).json({
        success: false,
        error:
...
router.get('/:id/export/word/step/:stepNumber', async (req: Request, res: Response) => {
  try {
    ...
    // Steps 5, 6 and 10 all need the module list from step4: 10 for module-level
